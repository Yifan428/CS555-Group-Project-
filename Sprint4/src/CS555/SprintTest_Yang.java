...
public class SprintTest_Yang {
	...
	public SprintTest_Yang() {
		...
		Sprint4_Yang.printINDIAndFAMTables(filename);
	}

	@Test
	public static void testUserStory23(List<Individual> allIndividuals) {
		List<Individual> individuals = allIndividuals;
		ArrayList<String> IndvName = new ArrayList<>();
		ArrayList<String> IndvBirthDate = new ArrayList<>();
		for (int i = 0; i < individuals.size(); i++) { 
			Individual individual = individuals.get(i); 
			String indvName = individual.getName();
			String indvBirthDate = individual.getBirthDate(); 
	...
	@Test 
	public static void testUserStory15(List<Family> allFamilies) { 
		List<Family> families = allFamilies; 
		int num = 0;
		for (int i = 0; i < families.size(); i++) { 
			Family family = families.get(i);  
			String famID = family.getId(); 
			List<String> childrenName = family.getChildrenname();
			if(!Sprint4_Yang.siblingsLessThan15(childrenName)) {
				System.out.println("ERROR: Family: US15: There are More than 15 siblings in the Families: " + famID);
				num = num +1;
			}
	...
	public static void testUserStory21(List<Family> allFamilies) { 
		...
		for (int i = 0; i < families.size(); i++) { 
			...
			if(Sprint4_Yang.validateHusGender(husbandId)) {
				System.out.println("ERROR: FAMILY: US21: Husband " + husbandId + " has the wrong gender");
				num = num +1;
			}
			if(Sprint4_Yang.validateWifeGender(wifeId)) {
	...
	public static void testUserStory22(List<Individual> allIndividuals, List<Family> allFamilies) {
		...
		for (int j = 0; j < families.size(); j++) { 
			...
			String famID = family.getId(); 
			FamID.add(famID);
			if(!Sprint4_Yang.uniqueId(FamID)) {
	...
	public static void testUserStory01(List<Individual> allIndividuals, List<Family> allFamilies) { 
		...
		List<Family> families = Sprint4_Yang.allFamilies; 
		for (int i = 0; i < families.size(); i++) { 
			Family family = families.get(i); 
